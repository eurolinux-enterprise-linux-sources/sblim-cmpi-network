...
CMPIObjectPath * _makePath_EthernetPort( const CMPIBroker * _broker,
  ...
  _OSBASE_TRACE(2,("--- _makePath_EthernetPort() called"));
  ...
  */ 
  if( !get_system_name() ) {   
    ...
    _OSBASE_TRACE(2,("--- _makePath_EthernetPort() failed : %s",CMGetCharPtr(rc->msg)));
    goto exit;
  }
 ...
 exit:
  _OSBASE_TRACE(2,("--- _makePath_EthernetPort() exited"));
  return op;                
}
...
CMPIInstance * _makeInst_EthernetPort( const CMPIBroker * _broker,
  ...
  _OSBASE_TRACE(2,("--- _makeInst_EthernetPort() called"));
  ...
  */
  if( !get_system_name() ) {   
    CMSetStatusWithChars( _broker, rc, 
			  CMPI_RC_ERR_FAILED, "no host name found" );
    _OSBASE_TRACE(2,("--- _makeInst_EthernetPort() failed : %s",CMGetCharPtr(rc->msg)));
    goto exit;
  }
  ...
  CMSetProperty( ci, "Caption", "Linux EthernetPort", CMPI_chars);
  CMSetProperty( ci, "Description", "This class represents instances of available Ethernet Ports.", CMPI_chars);
  ...
  CMSetProperty( ci, "PermanentAddress", sptr->mac_addr, CMPI_chars);  
 ...
 exit:
  _OSBASE_TRACE(2,("--- _makeInst_EthernetPort() exited"));
  return ci;
}
...
/* ---------------------------------------------------------------------------*/
/*                    end of cmpiOSBase_EthernetPort.c                        */
/* ---------------------------------------------------------------------------*/
