...
#include "cmpiOSBase_EthernetPort.h"
...
#ifdef CMPI_VER_100
#define OSBase_EthernetPortProviderSetInstance OSBase_EthernetPortProviderModifyInstance 
#endif
...
CMPIStatus OSBase_EthernetPortProviderCleanup( CMPIInstanceMI * mi, 
...
CMPIStatus OSBase_EthernetPortProviderEnumInstanceNames( CMPIInstanceMI * mi, 
  ...
  if( enum_all_netPorts( &lptr ) != 0 ) {
    CMSetStatusWithChars( _broker, &rc, 
			  CMPI_RC_ERR_FAILED, "Could not list ethernet ports." ); 
  ...
  if( lptr != NULL ) {
    for ( ; lptr && rc.rc == CMPI_RC_OK ; lptr = lptr->next) {
      if( lptr->sptr->type == 1 ) {
	op = _makePath_EthernetPort( _broker, ctx, ref,lptr->sptr, &rc );
...
CMPIStatus OSBase_EthernetPortProviderEnumInstances( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
           const CMPIObjectPath * ref, 
  ...
  if( lptr != NULL ) {
    for ( ; lptr && rc.rc == CMPI_RC_OK ; lptr = lptr->next) {
      if( lptr->sptr->type == 1 ) {
	ci = _makeInst_EthernetPort( _broker, ctx, ref, properties, lptr->sptr, &rc );
...
CMPIStatus OSBase_EthernetPortProviderGetInstance( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
  ...
  if( id == NULL ) {    
    CMSetStatusWithChars( _broker, &rc, 
			  CMPI_RC_ERR_FAILED, "Could not get EthernetPortID." ); 
    _OSBASE_TRACE(1,("--- %s CMPI GetInstance() failed : %s",_ClassName,CMGetCharPtr(rc.msg)));
    return rc;
  }

  cmdrc = get_netPort_data( CMGetCharPtr(id) , 1, &sptr );
  if( cmdrc != 0 || sptr == NULL ) {    
    CMSetStatusWithChars( _broker, &rc, 
			  CMPI_RC_ERR_NOT_FOUND, "EthernetPort does not exist." ); 
    _OSBASE_TRACE(1,("--- %s CMPI GetInstance() failed : %s",_ClassName,CMGetCharPtr(rc.msg)));
    return rc;
  }

  ci = _makeInst_EthernetPort( _broker, ctx, cop, properties, sptr, &rc );
...
CMPIStatus OSBase_EthernetPortProviderCreateInstance( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
           const CMPIObjectPath * cop, 
...
CMPIStatus OSBase_EthernetPortProviderSetInstance( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
           const CMPIObjectPath * cop,
           const CMPIInstance * ci, 
...
CMPIStatus OSBase_EthernetPortProviderDeleteInstance( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
           const CMPIObjectPath * cop) {
  CMPIStatus rc = {CMPI_RC_OK, NULL}; 
...
CMPIStatus OSBase_EthernetPortProviderExecQuery( CMPIInstanceMI * mi, 
           const CMPIContext * ctx, 
           const CMPIResult * rslt, 
           const CMPIObjectPath * ref, 
           const char * lang, 
...
CMPIStatus OSBase_EthernetPortProviderMethodCleanup( CMPIMethodMI * mi, 
           const CMPIContext * ctx, CMPIBoolean terminate) {
...
CMPIStatus OSBase_EthernetPortProviderInvokeMethod( CMPIMethodMI * mi,
           const CMPIContext * ctx,
           const CMPIResult * rslt,
           const CMPIObjectPath * ref,
...
CMInstanceMIStub( OSBase_EthernetPortProvider, 
                  OSBase_EthernetPortProvider, 
                  _broker, 
                  CMNoHook);

CMMethodMIStub( OSBase_EthernetPortProvider,
                OSBase_EthernetPortProvider, 
                _broker, 
                CMNoHook);
...
/* ---------------------------------------------------------------------------*/
/*                end of cmpiOSBase_EthernetPortProvider                      */
/* ---------------------------------------------------------------------------*/
